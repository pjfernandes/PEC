...
pec<-function(mde_leitura,pontos_leitura) {
  ...
  classe_50000_d<-(nrow(df[df$classe_50000=="NÃO SE APLICA",])/nrow(df))*100
  ...
  classe_100000_d<-(nrow(df[df$classe_100000=="NÃO SE APLICA",])/nrow(df))*100
 ...
 #CÁLCULO TV 1:250000
  ...
  for (i in df$V3) {
    if(i<=50){
      ...
      classe_250000<-c(classe_250000,"E")
    } else{
  ...
  classe_250000_d<-(nrow(df[df$classe_250000=="NÃO SE APLICA",])/nrow(df))*100
  
  
  #CONSOLIDANDO RESULTADO TV E CALCULANDO EMQ
  resultado_TV<-data.frame(escala=c(50000,100000,250000),a=c(classe_50000_a,classe_100000_a,classe_250000_a),b=c(classe_50000_b,classe_100000_b,classe_250000_b),c=c(classe_50000_c,classe_100000_c,classe_250000_c),nao_se_aplica=c(classe_50000_d,classe_100000_d,classe_250000_d))
